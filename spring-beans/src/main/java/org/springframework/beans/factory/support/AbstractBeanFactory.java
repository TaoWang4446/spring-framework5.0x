...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 * * @param指定要检索的bean的名称
	 * * @param需要获取所需的bean类型
	 * *使用显式参数创建bean实例时使用的@param参数
	 * *(只在创建新实例时应用，而不是检索现有实例时)
	 ...
	 * 4.原型模式的依赖检查
	 * 5.检查父工厂
	 * 6.将xml配置文件的GenericBeanDefinition 转换为 RootBeanDefinition。
	 * 7.寻找依赖
	 * 8.针对不同的scope 进行 bean的创建
	 * 9.类型转换
	 *
	 * 经过以上步骤，bean的加载就结束了，可以返回我们需要的bean了。
	 */
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		 ...
		 * 为什么首先会有这段代码呢？
		 * 因为在创建单例bean的是会存在依赖注入的情况，而在创建依赖的时候为了避免循环依赖，
		...
		if (sharedInstance != null && args == null) {
			...
			// 返回对应的实例，有时候存在如BeanFactory的情况并不是直接返回实例本身而是返回指定方法返回的实例
			...
			// A中有B的属性，B中有A的属性，那么当依赖注入的时候，就会产生当A还未创建完的时候因为对于B的创建
			// 再次返回创建A，造成循环依赖，也就是下面的情况
			...
			//containsBeanDefinition(beanName)判断当前配置文件是否包含当前beanName的配置
			...
			try {
				/**
				 * beanName 或许是别名 或许是 factorybean 或许是id
				 */
	...
	protected Object getObjectForBeanInstance(
		...
		//看英文注释，补充：想要获取FactoryBean，而不是getObject方法的话，传入的name要加前缀&
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
		// 加载FactoryBean
