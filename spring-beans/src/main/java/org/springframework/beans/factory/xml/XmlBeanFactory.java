 ...
 * 容器的基础
 *
 * Resource 完成对配置文件的封装之后
 * XmlBeanDefinitionReader 负责对 配置文件的读取 处理
 *
 *
 */
@Deprecated
...
public class XmlBeanFactory extends DefaultListableBeanFactory {
	...
	public XmlBeanFactory(Resource resource) throws BeansException {
		this(resource, null);//调用 XmlBeanFactory(Resource resource) 构造方法
	}
	 ...
	 * which must be parsable using DOM.
	 * @param resource XML resource to load bean definitions from
	 * @param parentBeanFactory parent bean factory 用于合并 工厂
	 * @throws BeansException in case of loading or parsing errors
	 * 构造方法内部 再次 调用 构造方法
	 */
	public XmlBeanFactory(Resource resource, BeanFactory parentBeanFactory) throws BeansException {
		super(parentBeanFactory);
		// 资源加载的 真正实现。 xmlBeanDifinationReader 加载数据就是在这里完成的
