 ...
 * 对资源文件的 读取 解析 及注册
 * ResourceLoader: 定义资源加载器，通过给定的 资源文件地址 返回对应的 Resource
 *
 * BeanDefinitionReader : 定义资源文件读取 并 转换为 BeanDefinition 的功能。
 *
 * resource文件 --> Document文档 --> Element解析
 *
 */
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * if none specified.
	 * 【返回EntityResolver 去使用 ，构建一个默认的解析器
	 * 如果没有指定。】
	 */
	...
	/**
	 * Load bean definitions from the specified XML file. 【从指定的xml资源 加载bean定义】
	 * @param encodedResource the resource descriptor for the XML file, 【xml的资源描述符】
	 * allowing to specify an encoding to use for parsing the file 【指定用于 解析 文件的编码】
	 * @return the number of bean definitions found 【找到的bean定义的 数量】
	 * @throws BeanDefinitionStoreException in case of loading or parsing errors
	 *  首先对 传入的 Resource 参数做封装（根据编码要求）
	 *  其次 通过 sax 读取xml的方式来准备InputResource对象
	 *  通过将准备的数据（以参数的形式）传入真正的核心处理方法 doLoadBeanDefinitions（）
	 */
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		try {
			//从 encodedResource 中获取已经封装的 Resource 对象，并再次从 Resource 中 获取 输入流
			InputStream inputStream = encodedResource.getResource().getInputStream();
			try {
				// InputSource 是 org.xml.sax.InputSource
	...
	/**
	 * Actually load bean definitions from the specified XML file. 【从指定的XML文件实际加载bean定义。】
	 * @param inputSource the SAX InputSource to read from 【sax读取】
	 * @param resource the resource descriptor for the XML file 【xml文件的资源描述】
	 * @return the number of bean definitions found 【发现bean定义的数量】
	 * @throws BeanDefinitionStoreException in case of loading or parsing errors
	 * @see #doLoadDocument 加载document
	 * @see #registerBeanDefinitions 注册bean定义
	 *
	 * 	1.获取对xml文件的验证模式
	 * 		dtd
	 * 		xsd
	 *  2.加载xml文件，得到对应的 Document
	 *  3.根据 Document注册bean信息。
	 *
	 *
	 *
	 */
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			...
			//把文件转换为document后，提取 bean 并注册bean。
	 ...
	 * 1.先创建 documentBuilderFactory
	 * 2. 然后通过 documentBuilderFactory 创建 documentBuilder 来解析 inputResource 返回 Document对象
	 *
	 */
	...
	/**
	 * Register the bean definitions contained in the given DOM document.【注册这bean定义 （包含在document对象中的）】
	 * Called by {@code loadBeanDefinitions}.
	 * <p>Creates a new instance of the parser class and invokes 【创建一个新的 解析器类实例 并 调用】
	 * {@code registerBeanDefinitions} on it. 【registerBeanDefinitions 在这个上面注册】
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		//使用默认的 BeanDefinitionDocumentReader实例化 BeanDefinitionDocumentReader
		//在实例化 BeanDefinitionDocumentReader 时候 会将 registerBeanDefinitions
		//默认使用 继承自 DefaultListableBeanFactory 的 子类。
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		//记录 统计前 BeanDefinition 的个数
		int countBefore = getRegistry().getBeanDefinitionCount();
		//加载 及 注册 bean registerBeanDefinitions
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//记录本次 BeanDefinition 的个数
