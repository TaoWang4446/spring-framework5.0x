...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		if (StringUtils.hasLength(nameAttr)) {//分割name属性
		...
		if (containingBean == null) {//如果不存在beanName 就根据 spring 提供的默认命名规则 为当前bean生成对应的beanName。
			checkNameUniqueness(beanName, aliases, ele);
		}

		//进一步解析其他所有属性， 并统一封装至 generateBeanDefination 类型的实例中
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		if (ele.hasAttribute(CLASS_ATTRIBUTE)) {
			className = ele.getAttribute(CLASS_ATTRIBUTE).trim();//解析 class属性
		}
		String parent = null;
		if (ele.hasAttribute(PARENT_ATTRIBUTE)) {//解析 parent属性
			parent = ele.getAttribute(PARENT_ATTRIBUTE);
		}

		try {
			//创建用于承载其他属性的AbstractBeanDefinition类型的 Generic未注册的BeanDefinition
			...
			parseReplacedMethodSubElements(ele, bd.getMethodOverrides());//解析 replace-method属性

			parseConstructorArgElements(ele, bd);//解析 构造函数 参数
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		NodeList nl = ele.getChildNodes();//获取当前节点的所有元素
	 ...
	 * 解析给定bean元素的替换方法子元素。
	 * 可以在运行时用 新的方法 替换 现有的方法
	 * 与 lookup不同的是：不但可以替换 返回的bean ，还可以 替换
	 */
	 ...
	 * @return the decorated bean definition
	 * 首先获取 属性 或者 元素的命名空间，判断该元素 是否适用于自定义标签的解析条件
	 * 找出自定义类型所对应的 NameSpaceHandler 并进一步做解析。
	 *
	 * xml文件 解析 装饰 完成以后，得到 bean定义 可以满足后续的需求了。
	 * 剩下的工作就是注册了。
	 */
